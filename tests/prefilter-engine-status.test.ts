...
describe("buildParamsPrompt engine availability", () => {
  it("buildEngineStatus exists in prefilter-prompts.ts", () => {
    ...
    assert.ok(
      ...
      "prefilter-prompts.ts must have buildEngineStatus function"
    );
  });
  ...
  it("buildParamsPrompt accepts engineStatus as parameter", () => {
    ...
    assert.ok(
      src.includes("engineStatus: string"),
      "buildParamsPrompt must accept engineStatus parameter"
    );
    // prefilter.ts calls buildEngineStatus and passes it to buildParamsPrompt
    const preSrc = readFileSync(PRE_PATH, "utf-8");
    assert.ok(
      preSrc.includes("buildEngineStatus"),
      "prefilter.ts must call buildEngineStatus"
    );
    assert.ok(
      preSrc.includes("buildParamsPrompt"),
      "prefilter.ts must call buildParamsPrompt"
    );
  });

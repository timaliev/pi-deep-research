...
describe("Engine allowlist — SettingsContext.enabledEngines", () => {
  ...
  it("defaults to duckduckgo plus searxng", () => {
    ...
    assert.ok(
      src.includes('"duckduckgo"') && src.includes('"searxng"'),
      "default must include duckduckgo and searxng",
    );
  });
  ...
  it("reads from DEEP_RESEARCH_ENABLED_ENGINES env var", () => {
    ...
    assert.ok(
      src.includes("DEEP_RESEARCH_ENABLED_ENGINES"),
      "must read enabledEngines from env var",
    );
  });

  it("prefilter-prompts buildEngineStatus filters by allowlist", () => {
    const src = readFileSync(
      join(import.meta.dirname ?? ".", "..", "extension", "prefilter-prompts.ts"),
      "utf-8",
    );
    // buildEngineStatus must now accept enabledEngines param or filter internally
    assert.ok(
      src.includes("enabled") || src.includes("allowlist"),
      "buildEngineStatus must reference allowlist",
    );
  });
});

...
describe("PostProcessor pipeline", () => {
  it("orchestrator has PostProcessor interface", () => {
    assert.ok(src.includes("PostProcessor") || src.includes("postProcessor"), "must have PostProcessor concept");
  });

  it("buildDoneResult iterates a pipeline", () => {
    assert.ok(src.includes("pipeline") || src.includes("processors"), "buildDoneResult must use a processor pipeline");
  });
});
